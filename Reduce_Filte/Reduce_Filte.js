...
function avgAge(persons) {
    var red = (x, y) => x + y.age;
    var result = persons.reduce(red, 0) / persons.length;
    return result;
}
/*
2
...
function longestName(arr) {
    var result = arr.reduce(function (total, id) {

        if (total.length < (id.name.first + id.name.last).length) {
             total = id.name.first + " " + id.name.last 
            }
        return total
    },
